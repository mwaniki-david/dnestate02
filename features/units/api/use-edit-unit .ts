...
type ResponseType = InferResponseType<typeof client.api.unit[":id"]["$patch"]>;
type RequestType = InferRequestType<typeof   client.api.unit[":id"]["$patch"]>["json"];

export const useEdiunit = (id?: string) => {
    const queryClient = useQueryClient();
  const mutation = useMutation<ResponseType, Error, RequestType>({
    mutationFn: async (json) => {
      const response = await client.api.unit[":id"]["$patch"]({
        param: { id },
         json
         });
