...
type ResponseType = InferResponseType<typeof client.api.buildingOwner.$post>;
type RequestType = InferRequestType<typeof   client.api.buildingOwner.$post>["json"];

export const useCreatbuildingOwner = () => {
    const queryClient = useQueryClient();
  const mutation = useMutation<ResponseType, Error, RequestType>({
    mutationFn: async (json) => {
      const response = await client.api.buildingOwner.$post({ json });
      return await response.json();
    },
    onSuccess: () => {
      toast.success("buildingOwner Created");
      queryClient.invalidateQueries({ queryKey: ["buildingOwner"] });
    },
    onError: () => {
      toast.error("Failed to createbuildingOwner");
    },
  });
  return mutation;
};
